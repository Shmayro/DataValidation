...
def separateNum(tab):
    ...
    for j in range(0, len(tab)):
        if tab[j].isdigit() == False and len(tab[j]) > 2:
            ...
            ll = len(tab[j])
            x = tab[j]
            if x != F and S not in ['BIS', 'TER', 'QUATER', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'Q', 'T']:
                ind = x.find(F)
                if ind == 0:
...
ROADKEY = ['AUTOROUTE', 'D', 'LIEUX-DITS', 'LIEUX-DIT', 'LIEU-DIT', 'RD', 'BL', 'ALLEES', 'MONTE', 'ALLES', 'AVNUE', 'PASSAGE', 'HAMEAU', 'ESPLANADE', 'ROAD', 'QUAI', 'BOULEVARD', 'RUE', 'AVENUE', 'CHEMIN', 'IMPASSE', 'PLACE',
           'ROUTE', 'ALLEE', 'COURS', 'VOIE', 'CHAUSSE', 'CHAUSSEE', 'TRAVERSE', 'LIEUDIT', 'PROMENADE', 'SQUARE', 'COTEAU', 'TERRE', 'CARREFOUR', 'CLOS', 'BALCON', 'CAVEE',
           'BOUCLE', 'ROUTES', 'PARVIS', 'LIEU', 'ALLE', 'CD']
HOUSEKEY = ['AERODROME', 'ATRIUM', 'EUROFRET', 'MOLE', 'TECHNOPARC', 'HOTEL', 'CS', 'CASE', 'B.P.', 'AIRPORT', 'ZA', 'ZAC', 'ZONE', 'PARC',
            'BATIMENT', 'IMMEUBLE', 'CITE', 'PARK', 'ZI',
                        'AEROPORT', 'LOTISSEMENT', 'BP', 'ZL', 'CE', 'BAT1', 'ENTREPOT', 'CHATEAU',
                        'TERMINAL', 'RESIDENCE', 'Z.A.C.', 'CS', 'ZAE', 'PA', 'BAT.', 'ECOPOLE', 'DOMAINE', 'CENTRE', 'CARGOPORT',
                        'BP', 'PO', 'BUREAU', 'ETAGE', 'APPARTEMENT', 'RSIDENCE', 'DOMAINE', 'ZAL', 'QUARTIER', 'P.A.', 'PLATE-FORME']

LKEY = ['LIEUX-DIT', 'AUTOROUTE', 'FERME', 'LIEUX-DITS', 'LIEU-DIT', 'RD', 'USINE', 'CS', 'CD', 'ALLE', 'ROUTES', 'AERODROME', 'ESPLANADE', 'ROAD', 'QUAI', 'BOULEVARD', 'RUE', 'AVENUE', 'CHEMIN', 'IMPASSE', 'PLACE',
        'ROUTE', 'ALLEE', 'COURS', 'VOIE', 'CHAUSSE', 'CHAUSSEE', 'TRAVERSE', 'LIEUDIT', 'PROMENADE', 'ENTREE', 'GATE', 'PORTE',
        'BAT.', 'BATIMENT', 'BUILDING', 'TOUR', 'IMMEUBLE',
                'RESIDENCE', 'MOLE', 'AEROGARE', 'GARE', 'ENTREPOT', 'CARGOPORT', 'PORT', 'AIRPORT', 'AEROPORT', 'EUROAIRPORT',
                'EUROFRET', 'TECHNOPARC', 'PARK', 'CENTRE', 'PARC', 'DOMAINE', 'ZONE', 'ZL', 'ZIF', 'ZAE', 'Z.I.P.', 'Z.A.C.',
                'ZAC', 'PA', 'Z.A.C', 'ZI', 'Z.I.', 'P.A.'
                'ZA', 'BP', 'PO', 'C/O', 'APPARTEMENT', 'ETAGE', 'BUREAU', 'ATRIUM', 'RSIDENCE', 'PLA', 'DEPOT', 'HANGAR', 'ACTIPOLE', 'MIN', 'DOMAINE', 'ALLEES', 'ZAL', 'QUARTIER', 'BL', 'VILLAGE', 'POLE', 'TECHNOPOLE']

L0KEY = ['GATE', 'PORTE', 'BUREAU', 'ETAGE', 'APPARTEMENT', 'ATRIUM', 'RDC']
L1KEY = ['BT', 'BAT.', 'BATIMENT', 'BUILDING',
         'TOUR', 'ENTREE', 'BT.', 'PLA', 'ILOT']
L2KEY = ['IMMEUBLE', 'RESIDENCE', 'MOLE', 'LOTISSEMENT',
         'RSIDENCE', 'ENTREPOT', 'DEPOT', 'HANGAR', 'USINE']
L3KEY = ['AEROGARE', 'GARE', 'AERODROME', 'EUROGARE', 'ARODROME']
L4KEY = ['CARGOPORT', 'PORT']
L5KEY = ['AIRPORT', 'AEROPORT', 'EUROAIRPORT', 'TERMINAL', 'AROPORT']
L6KEY = ['EUROFRET', 'TECHNOPARC', 'PARK', 'CENTRE', 'PARC', 'DOMAINE', 'ZONE', 'ZL', 'ZIF', 'ZAE', 'Z.I.P.', 'Z.A.C.',
         'ZAC', 'PA', 'Z.A.C', 'ZI', 'Z.I.', 'ZA', 'RVSL', 'FORUM', 'ACTIPOLE', 'MIN', 'ECOPARC', 'MARCHE', 'ZIL', 'PLATEFORME',
         'ZCI', 'DOMAINE', 'ZAL', 'QUARTIER', 'VILLAGE', 'POLE', 'TECHNOPOLE', 'P.A.', 'FERME', 'PLATE-FORME']
...
def ParsingROAD(Addr):
    ...
    if PR != -1:
        ...
        ######################################################
        elif PR > 0 and L[PR - 1].isdigit() == False and filter(str.isdigit, L[PR - 1]) != '':
            if PH == []:
                ...
                ROAD = " ".join(str(x) for x in L2)
        ########################################################
        elif (PR > 0 and L[PR - 1] in ['BIS', 'TER', 'QUATER', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'Q', 'T'] and (L[PR - 2].isdigit() == True)):
            if PH == []:
        ...
        if 'EME' not in S:
            while k < len(L) and T == False:
                if L[k].isdigit() == True or numT(L[k]) == True:
                    ...
                    k = k + 1
            if T == True:
                if L[k+1] in ['BIS', 'TER', 'QUATER', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'Q', 'T']:
...
def Address_Standardization(Address):
    ...
    if HouseNum == 'NONE':
        ...
        if p1 != -1 and EXTBUILDING[p1-1].isdigit() == True and EXTBUILDING[p1+1].isdigit() == True and p1 == len(EXTBUILDING)-2:
        ...
        elif p2 != -1 and POILOGISTIC[p2-1].isdigit() == True and POILOGISTIC[p2+1].isdigit() == True and p2 == len(POILOGISTIC)-2:
        ...
        elif p3 != -1 and ZONE[p3-1].isdigit() == True and ZONE[p3+1].isdigit() == True and p3 == len(ZONE)-2:
...
def RefDBMatching(CompanyName, Addr, INBUILDING, EXTBUILDING, POILOGISTIC, ZONE, HouseNum, RoadName, POBOX, city, country, ADDITIONAL):
    ...
    #city = R[1]
    #country = R[2]
    z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.INBUILDING': INBUILDING,
                         'company.Address.EXTBUILDING': EXTBUILDING,
                         'company.Address.POILOGISTIC': POILOGISTIC, 'company.Address.ZONE': ZONE,
                         'company.Address.HouseNum': HouseNum,
                         'company.Address.RoadName': RoadName, 'company.Address.POBOX': POBOX,
                         'company.Address.city': city, 'company.Address.country': country})
    # 2
    ...
    if (INBUILDING != 'NONE' and EXTBUILDING != 'NONE' and
        ...
        z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.INBUILDING': INBUILDING,
                             ...
                             'company.Address.POILOGISTIC': POILOGISTIC, 'company.Address.ZONE': ZONE,
                             'company.Address.HouseNum': HouseNum,
                             'company.Address.RoadName': RoadName, 'company.Address.city': city,
    ...
    if (INBUILDING != 'NONE' and EXTBUILDING != 'NONE' and
        ...
        z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.INBUILDING': INBUILDING,
                             'company.Address.EXTBUILDING': EXTBUILDING,
                             'company.Address.POILOGISTIC': POILOGISTIC, 'company.Address.HouseNum': HouseNum,
                             'company.Address.RoadName': RoadName, 'company.Address.city': city,
                             'company.Address.country': country})
    # 5
    if (INBUILDING != 'NONE' and EXTBUILDING != 'NONE' and
        ...
        z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.INBUILDING': INBUILDING,
                             'company.Address.EXTBUILDING': EXTBUILDING,
                             'company.Address.HouseNum': HouseNum, 'company.Address.RoadName': RoadName,
                             'company.Address.city': city, 'company.Address.country': country})
    # 6
    if (EXTBUILDING != 'NONE' and HouseNum != 'NONE' and RoadName != 'NONE'
            and city != 'NONE') or (z.count() == 0):
        z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.EXTBUILDING': EXTBUILDING,
                             'company.Address.HouseNum': HouseNum, 'company.Address.RoadName': RoadName,
                             'company.Address.city': city, 'company.Address.country': country})
    ...
    if (ZONE != 'NONE' and HouseNum != 'NONE' and RoadName != 'NONE'
            and city != 'NONE') or (z.count() == 0):
        z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.ZONE': ZONE,
                             'company.Address.HouseNum': HouseNum, 'company.Address.RoadName': RoadName,
                             'company.Address.city': city, 'company.Address.country': country})
    ...
    if (HouseNum != 'NONE' and RoadName != 'NONE' and city != 'NONE') or (z.count() == 0):
        z = collection.find({'company.Name': {'$regex': CompanyName}, 'company.Address.HouseNum': HouseNum,
                             'company.Address.RoadName': RoadName, 'company.Address.city': city,
                             'company.Address.country': country})
    ...
    if (city != 'NONE') or (z.count() == 0):
        z = collection.find(
            {'company.Name': {'$regex': CompanyName},  'company.Address.city': city})
    # if (z.count()==0):
    #	z = collection.find({'company.Name': {'$regex': CompanyName}})
    D = 'NONE'
    Nbr = z.count()
    ...
    if Nbr > 0:
        for k in range(0, Nbr):
            D = {'CompanyName': z[k]['company']['Name'], 'INBUILDING': z[k]['company']['Address']['INBUILDING'],
                 'EXTBUILDING': z[k]['company']['Address']['EXTBUILDING'],
                 'POILOGISTIC': z[k]['company']['Address']['POILOGISTIC'], 'ZONE': z[k]['company']['Address']['ZONE'],
                 'HouseNum': z[k]['company']['Address']['HouseNum'], 'RoadName': z[k]['company']['Address']['RoadName'],
                 'POBOX': z[k]['company']['Address']['POBOX'], 'zipcode': z[k]['company']['Address']['zipcode'],
                 'city': z[k]['company']['Address']['city'], 'country': z[k]['company']['Address']['country'], 'ADDITIONAL': z[k]['company']['Address']['ADDITIONAL']}
            LLIST.insert(k, D)
    return LLIST
